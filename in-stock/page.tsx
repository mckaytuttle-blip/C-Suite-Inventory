    ...
    <div className="page">
      ...
      <section className="panel">
        ...
        <p className="panel-sub">
          ...
          On Hand / Available / Committed reflect the most recent snapshot for each component
          (not always today, if a run was missed) — Committed is On Hand minus Available, Zoho&apos;s
          only per-item signal for stock already spoken for by open orders or assemblies.
        </p>
        <table>
          <thead>
            <tr>
          ...
          <tbody>
            {sorted.map((c) => (
              <tr key={c.name}>
                ...
                <td className="numeric" title={c.latestSnapshotDate ? `As of ${c.latestSnapshotDate}` : undefined}>
                  {c.latestStockOnHand ?? "—"}
                </td>
                <td className="numeric" title={c.latestSnapshotDate ? `As of ${c.latestSnapshotDate}` : undefined}>
                  {c.latestAvailableStock ?? "—"}
                </td>
                <td className="numeric" title={c.latestSnapshotDate ? `As of ${c.latestSnapshotDate}` : undefined}>
                  {c.latestCommitted ?? "—"}
                </td>
